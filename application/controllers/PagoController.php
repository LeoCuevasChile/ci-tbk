...
class PagoController extends CI_Controller {
    ...
    public function __construct() 
    {
        ...
        $conf = $this->transbank->config();
        $conf->setEnvironment('INTEGRACION');
        $conf->setCommerceCode('597020000541');
        $conf->setPrivateKey( $this->private_key() );
        $conf->setPublicCert( $this->public_cert() );
        $conf->setWebpayCert( $this->webpay_cert() );
        
        $this->transbank->init( $conf );
    }
    ...
    public function index()
    {
        ...
        $this->transbank->setMonto($monto);

		$this->transbank->setReturnUrl('pago/resultado');
		$this->transbank->setEndUrl('pago/exito');
		$this->transbank->setErrorUrl('pago/error');
...
/* End of file PagoController.php */
